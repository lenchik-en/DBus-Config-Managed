...
void ConfigurationObject::reloadConfig() {
  ...
  version_ =
      configuration_.value("General").toMap().value("version").toString();
...
void ConfigurationObject::ChangeConfiguration(const QString& key,
...
void ConfigurationObject::updateJsonFileIfNeeded(const QString& key,
                                                 const QVariant& value) {
  // Только для приложения confManagerApplication1
  if (!configFilePath_.contains("confManagerApplication1.conf")) return;
  ...
  else if (section == "General" && subkey == "TimeoutPhrase" &&
